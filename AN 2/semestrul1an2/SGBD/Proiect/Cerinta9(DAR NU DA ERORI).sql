...
--Numele institutiilor de invatamant pe care le au absolvit membrii familiei cu cel mai mare contract la co companie data
...
CREATE OR REPLACE PROCEDURE procedura_cerinta_9
    ...
    WHERE lower(c.nume) = lower(v_cumparator)
        AND ccu.suma = (SELECT max(suma) FROM contract_cumparare ccu2 WHERE ccu2.id_familie = l.id_familie);
...
/
BEGIN
    procedura_cerinta_9('dada'); -- ar trebui sa dea eroare, dar nu da
END;
/
