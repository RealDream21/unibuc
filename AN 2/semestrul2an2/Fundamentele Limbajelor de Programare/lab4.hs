...
substaexp (Nu a) str replaceWithThis = (Nu a)
substaexp (Qid a) str replaceWithThis = (Qid str)
substaexp (PlusE exp1 exp2) str replaceWithThis = PlusE (substaexp exp1 str replaceWithThis) (substaexp exp2 str replaceWithThis)
substaexp (TimesE exp1 exp2) str replaceWithThis = TimesE (substaexp exp1 str replaceWithThis) (substaexp exp2 str replaceWithThis)
substaexp (DivE exp1 exp2) str replaceWithThis = DivE (substaexp exp1 str replaceWithThis) (substaexp exp2 str replaceWithThis)
...
valueassn (AndEX ass1 ass2) state = (valueassn ass1 state) && (valueassn ass2 state)
valueassn (DisjInfX xs) state = any (`valueassn` state) xs
--valueassn (DisjInfX (x: xs)) state = AICI NU STIU CE EXPRESIE VINE INTRE ELE
...
convassn :: BExp -> Assn
convassn (BE a) = BEX a
convassn (LE exp1 exp2) = (LEX exp1 exp2)
convassn (NotE exp1) = NotEX (convassn exp1)
convassn (AndE exp1 exp2) = AndEX (convassn exp1) (convassn exp2)
...
substassn (BEX a) str aexp = (BEX a)
substassn (LEX exp1 exp2) str aexp = LEX (substaexp exp1 str aexp) (substaexp exp2 str aexp)
substassn (NotEX exp1) str aexp = NotEX (substassn (exp1) str aexp)
substassn (AndEX exp1 exp2) str aexp = AndEX (substassn(exp1) str aexp) (substassn(exp2) str aexp) 
substassn (DisjInfX xs) str aexp = DisjInfX (map (\assn -> substassn assn str aexp) xs)
