...
ackermannf = fix (\f x y -> aux x y)
aux x y
    | x == 0 && (y /= 0) = y + 1
    | x /= 0 && (y == 0) = aux (x - 1) 1
    | otherwise = aux (x - 1) (aux x (y - 1))
...
evenoddf = fix (\f nr -> ( fix (\c -> evenp nr), fix(\p -> oddp nr)))
...
instance Functor StmtF where
    fmap eval SkipF = skip
    fmap eval (AtrEF t e) = eval e
    fmap eval (SeqF s1 s2) = fmap (eval s1) (eval s2)
