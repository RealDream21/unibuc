...
data Term = Variable String | FuncSym String [Term]
    deriving (Eq, Show)
-}
substitute :: [Equ] -> Term -> Term --ec sunt de forma o variabila = un termen
substitute [] t = t
substitute (Equ (Variable x) s : es) t = substitute es (subst t x s)  --inlocuim pe x cu s

data AtomicRel = AtomicRel String [Term]
    deriving (Eq, Show)

-- variables of a relational atomic formula
vara :: AtomicRel -> [String] --curs 5 slide 12 
vara (AtomicRel p ts) = union (map var ts) 

-- substitution for an atomic formula
substitutea :: [Equ] -> AtomicRel -> AtomicRel
substitutea es (AtomicRel p ts) = AtomicRel p (substitute es <$> ts)
...
unifya :: AtomicRel -> AtomicRel -> AllResult --daca cele 2 relatii atomice au ambele liste de termeni => lista de rez va fi pe rand egalitatea dintre termenii lor
...
--varcl (Clause at ats) = (vara at) ++ (map (concat . vara) ats)
...
progm = [
    Clause (vmul vx termz termz) [], 
    Clause (vmul vx (terms vy) vz) [vmul vx vy vw, vadd vw vx vz]
    ]
prog  = proga ++ progm
