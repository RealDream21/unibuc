...
update str nr [] = [(str, nr)]
update str nr (x:xs)
    | str == (fst x) = (fst x, nr) : (update str nr xs)
    | otherwise = (fst x, snd x) : update str nr xs
...
value (Nu a) lista = a
value (Qid str) lista = recall str lista
value (PlusE exp1 exp2) lista = (value exp1 lista) + (value exp2 lista)
value (TimesE exp1 exp2) lista = (value exp1 lista) * (value exp2 lista)
value (DivE exp1 exp2) lista = (value exp1 lista) `div` (value exp2 lista)
...
valueb (BE a) lista = a
valueb (LE exp1 exp2) lista = (value exp1 lista) <= (value exp2 lista)
valueb (NotE exp1) lista = not (valueb exp1 lista)
valueb (AndE exp1 exp2) lista = (valueb exp1 lista) && (valueb exp2 lista)
...
bssos (AtrE str exp) stare = update str (value exp stare) stare
bssos (IfE exp cod1 cod2) stare
    | (valueb exp stare) == True = bssos cod1 stare
    | otherwise = bssos cod2 stare
bssos (Seq cod1 cod2) stare = bssos cod2 (bssos cod1 stare) 
bssos (WhileE exp1 cod) stare
    | (valueb exp1 stare) == True = bssos (WhileE exp1 cod) stare
    | otherwise = stare
bssos skip stare = stare


--small step
sssos1 :: (Stmt, [(String, Int)]) -> (Stmt, [(String, Int)])
sssos1 = undefined
{-sssos1 ((AtrE str exp), stare) = (Skip, update str (value exp stare) stare)
sssos1 ((Seq exp1 exp2), stare) = (sssos1 (exp1, stare))
-}
