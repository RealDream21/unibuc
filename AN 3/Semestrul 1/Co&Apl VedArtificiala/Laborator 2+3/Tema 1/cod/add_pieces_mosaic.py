...
def get_sorted_distances(mean_color_pieces, patch):
    ...
    if params.criterion == 'aleator':
        for i in range(params.num_pieces_vertical):
            for j in range(params.num_pieces_horizontal):
                index = np.random.randint(low = 0, high = N, size = 1) # index piesa random
        ...
        for i in range(params.num_pieces_vertical):
            for j in range(params.num_pieces_horizontal):
                ...
                sorted_indices = get_sorted_distances(mean_color_pieces, mean_color_patch)
